import{z as K,J,K as q,N,u as P,r as e,V as M,W as B,U as ee,X as H,e as T,Y as te,Z as F,_ as Q,C as re,$ as ae,a0 as ne,a1 as oe,Q as se,a2 as k,a3 as ie,a4 as le,a5 as ce}from"./index-B2pfPin8.js";const ue=new J(-1,1,1,-1,0,1);class me extends q{constructor(){super(),this.setAttribute("position",new N([-1,3,0,-1,-1,0,3,-1,0],3)),this.setAttribute("uv",new N([0,2,0,0,2,0],2))}}const pe=new me;class fe{constructor(a){this._mesh=new K(pe,a)}dispose(){this._mesh.geometry.dispose()}render(a){a.render(this._mesh,ue)}get material(){return this._mesh.material}set material(a){this._mesh.material=a}}const de=()=>parseInt(ae.replace(/\D+/g,"")),ve=de(),A=new M,W=new Q,b=new M,z=new M,D=new M,Z=new oe;e.forwardRef(({autoTransform:i=!0,matrix:a,axisLock:p,dragLimits:o,onHover:s,onDragStart:m,onDrag:n,onDragEnd:y,children:u,dragConfig:l,...r},c)=>{const w=P(d=>d.controls),{camera:h,size:v,raycaster:f,invalidate:E}=P(),t=e.useRef(null),_=ne({onHover:({hovering:d})=>s&&s(d??!1),onDragStart:({event:d})=>{w&&(w.enabled=!1);const{point:x}=d;t.current.matrix.decompose(A,new se,new M),b.copy(x),z.copy(b).sub(A),m&&m(A),E()},onDrag:({xy:[d,x],intentional:S,event:O})=>{if(!S)return;l&&l.preventOverlap&&O.preventDefault();const L=(d-v.left)/v.width*2-1,X=-((x-v.top)/v.height)*2+1;if(W.set(L,X),f.setFromCamera(W,h),!p)h.getWorldDirection(D).negate();else switch(p){case"x":D.set(1,0,0);break;case"y":D.set(0,1,0);break;case"z":D.set(0,0,1);break}Z.setFromNormalAndCoplanarPoint(D,b),f.ray.intersectPlane(Z,b);const U=t.current.matrix.clone(),I=t.current.matrixWorld.clone(),g=new M(b.x-z.x,b.y-z.y,b.z-z.z);if(o&&(g.x=o[0]?Math.max(Math.min(g.x,o[0][1]),o[0][0]):g.x,g.y=o[1]?Math.max(Math.min(g.y,o[1][1]),o[1][0]):g.y,g.z=o[2]?Math.max(Math.min(g.z,o[2][1]),o[2][0]):g.z),i){t.current.matrix.setPosition(g);const C=t.current.matrix.clone().multiply(U.invert()),R=t.current.matrix.clone().multiply(I.invert());n&&n(t.current.matrix,C,t.current.matrixWorld,R)}else{const C=new F().copy(t.current.matrix);C.setPosition(g);const R=C.clone().multiply(U.invert()),Y=C.clone().multiply(I.invert());n&&n(C,R,t.current.matrixWorld,Y)}E()},onDragEnd:()=>{w&&(w.enabled=!0),y&&y(),E()}},{drag:{filterTaps:!0,threshold:1,...typeof l=="object"?l:{}}});return e.useImperativeHandle(c,()=>t.current,[]),e.useLayoutEffect(()=>{a&&(t.current.matrix=a)},[a]),e.createElement("group",{ref:t,..._(),matrix:a,matrixAutoUpdate:!1,...r},u)});function V(i){const a=window.devicePixelRatio||1;return Math.round(i*a)/a}const $=e.createContext(null),j=new WeakMap;function he(){return e.useContext($)}e.forwardRef(({children:i,style:a,...p},o)=>{const s=he(),m=e.useRef(null);e.useImperativeHandle(o,()=>m.current,[]);const{width:n,height:y}=P(r=>r.size),u=e.useContext(k),l=e.useMemo(()=>{let r=j.get(s.fixed);return r||(r=ie.createRoot(s.fixed),j.set(s.fixed,r)),r},[s.fixed]);return T(()=>{if(s.delta>s.eps){let r=s.horizontal?-n*(s.pages-1)*s.offset:0,c=s.horizontal?0:y*(s.pages-1)*-s.offset;s.pixelPerfect&&(r=V(r),c=V(c)),m.current.style.transform=`translate3d(${r}px,${c}px,0)`}}),l.render(e.createElement("div",{ref:m,style:{...a,position:"absolute",top:0,left:0,willChange:"transform"},...p},e.createElement($.Provider,{value:s},e.createElement(k.Provider,{value:u},i)))),null});e.forwardRef(({name:i,defaultValue:a,normalized:p,usage:o=le,type:s="float"},m)=>{const n=e.useRef(null);e.useImperativeHandle(m,()=>n.current,[]),e.useLayoutEffect(()=>{const u=n.current.__r3f.parent.object;u.geometry.attributes[i]=n.current;const l=Array.isArray(a)?a:[a],r=Array.from({length:u.userData.limit},()=>l).flat();switch(s){case"float":n.current.array=new Float32Array(r);break;case"int":n.current.array=new Int32Array(r);break;case"uint":n.current.array=new Uint32Array(r);break;case"short":n.current.array=new Int16Array(r);break;case"ushort":n.current.array=new Uint16Array(r);break;case"byte":n.current.array=new Uint8Array(r);break}return n.current.itemSize=l.length,n.current.count=r.length/n.current.itemSize,()=>{delete u.geometry.attributes[i]}},[i]);let y=0;return T(()=>{const u=n.current.__r3f.parent.object;if(u.userData.frames===1/0||y<u.userData.frames){for(let l=0;l<u.userData.instances.length;l++){const c=u.userData.instances[l].current[i];c!==void 0&&(n.current.set(Array.isArray(c)?c:typeof c.toArray=="function"?c.toArray():[c],l*n.current.itemSize),n.current.needsUpdate=!0)}y++}}),e.createElement("instancedBufferAttribute",{ref:n,usage:o,normalized:p})});e.createContext(null);const G="__drei_view_tunnel__";globalThis[G]||(globalThis[G]=ce());e.createContext(null);function xe(){const i=P(a=>a.gl);return e.useEffect(()=>(i.shadowMap.autoUpdate=!1,i.shadowMap.needsUpdate=!0,()=>{i.shadowMap.autoUpdate=i.shadowMap.needsUpdate=!0}),[i.shadowMap]),null}class ye extends H{constructor(){super({uniforms:{depth:{value:null},opacity:{value:1},attenuation:{value:2.5},anglePower:{value:12},spotPosition:{value:new M(0,0,0)},lightColor:{value:new re("white")},cameraNear:{value:0},cameraFar:{value:1},resolution:{value:new Q(0,0)}},transparent:!0,depthWrite:!1,vertexShader:`
